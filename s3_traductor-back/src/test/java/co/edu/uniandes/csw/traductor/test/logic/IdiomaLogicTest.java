...
public class IdiomaLogicTest {
    ...
    public void getIdiomaTest(){
        try{
            idiomaLogic.getIdioma(Long.MAX_VALUE);
            Assert.fail("El idioma no debería existir");
        }
        catch(BusinessLogicException e){
            Assert.assertTrue("Deberia haber un mensaje de excepcion", e.getMessage().length() != 0);
        }
        IdiomaEntity a=null;
        try{
            a=idiomaLogic.getIdioma(data.get(1).getId());
        }
        catch(BusinessLogicException e){
            Assert.assertTrue("Deberia haber un mensaje de excepcion", e.getMessage().length() != 0);
        }
    ...
    public void deleteIdiomaTest(){
        try{
            ...
            Assert.assertTrue("Deberia haber un mensaje de excepcion", b.getMessage().length() != 0);
        }
    }
}
